// src/Pages/Admin/components/modals/EditBookingModal.jsx
...
const EditBookingModal = ({ booking, onClose, onSave, formatCurrency, BASE_URL }) => {
    ...
    useEffect(() => {
        if (booking) {
            setFormData({
                ...
                agentId: booking.agentId?.toString() || '',
    ...
    // fetchCustomers/vehicles/agents are defined inside the component and
    // we only want to run this effect when BASE_URL changes.  The eslint
    // rule would normally ask us to add those functions to the dependency
    // array, but they never change so it's safe to suppress the warning.
    ...
    useEffect(() => {
        if (BASE_URL) {
            ...
            console.error('BASE_URL is not provided to EditBookingModal');
            setFetchError('Configuration error: BASE_URL is missing');
        }
    }, [BASE_URL]);
    ...
    }, [formData.driverStatus, formData.gpsIncluded, formData.childSeatIncluded, selectedVehicleDetails, formData.pickupDate, formData.dropOffDate]);
    ...
    if (!booking) {
        console.error('EditBookingModal: No booking data provided');
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
                    <div className="text-center">
                        ...
                        <p className="text-gray-600 mb-4">No booking data provided</p>
                        <button
                            onClick={onClose}
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log('Updating booking:', submitData);
    ...
    const formatCurrencySafe = (amount) => {
        try {
            ...
            // `formatCurrency` may throw; we don't need the error object here.
                ...
                <div className="p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold text-gray-800">
                            Edit Booking #{booking?.id ? `BK${String(booking.id).padStart(4, '0')}` : ''}
                        </h2>
                            ...
                            >
                                ...
                                {filteredVehicles.map(vehicle => (
                                    <option key={vehicle.id} value={vehicle.id}>
                                        #VEH000{vehicle.id} - {vehicle.makeModel} ({vehicle.regNumber}) - {formatCurrencySafe(vehicle.dailyRentalPrice || 0)}/day
                                    </option>
                                ))}
                            </select>
                        ...
                        {selectedVehicleDetails && (
                            <div className="bg-teal-50 p-3 rounded-lg text-sm">
                                ...
                                <p className="text-teal-700 font-semibold mt-1">
                                    Base Rate: {formatCurrencySafe(selectedVehicleDetails.dailyRentalPrice || 0)}/day
                                </p>
                            </div>
                        )}
                            ...
                            <div className="bg-teal-50 p-4 rounded-lg">
                                ...
                                <div className="space-y-1 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Base Rate ({days} days × {formatCurrencySafe(selectedVehicleDetails.dailyRentalPrice || 0)}/day)</span>
                                        <span className="font-medium">{formatCurrencySafe(days * (selectedVehicleDetails.dailyRentalPrice || 0))}</span>
                                    </div>
                                    
                                    {formData.driverStatus === 'WITH_DRIVER' && (
                                        <div className="flex justify-between text-gray-600">
                                            <span>Driver Charge ({days} days × Rs. {EXTRA_CHARGES.DRIVER}/day)</span>
                                            <span className="font-medium">{formatCurrencySafe(days * EXTRA_CHARGES.DRIVER)}</span>
                                        </div>
                                    )}
                                    ...
                                    {formData.gpsIncluded && (
                                        <div className="flex justify-between text-gray-600">
                                            ...
                                            <span className="font-medium">{formatCurrencySafe(days * EXTRA_CHARGES.GPS)}</span>
                                        </div>
                                    )}
                                    ...
                                    {formData.childSeatIncluded && (
                                        <div className="flex justify-between text-gray-600">
                                            ...
                                            <span className="font-medium">{formatCurrencySafe(days * EXTRA_CHARGES.CHILD_SEAT)}</span>
                                        </div>
                                    )}
                                    ...
                                    <div className="border-t border-teal-200 pt-2 mt-2">
                                        <div className="flex justify-between font-bold text-teal-800">
                                            ...
                                            <span>{formatCurrencySafe(formData.totalPrice)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
