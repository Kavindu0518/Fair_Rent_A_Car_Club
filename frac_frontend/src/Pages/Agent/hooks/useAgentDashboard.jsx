...
// //     // Handle vehicle deletion with toast
...
// //         try {
// //             await axios.delete(`${BASE_URL}/api/v1/vehicle/delete/${vehicleToDelete.id}`, {
// //                 headers: { 'Authorization': `Bearer ${localStorage.getItem('agentToken')}` }
// //             });
            
// //             await fetchAgentVehicles(); // Refresh vehicles list
// //             closeDeleteModal();
...
//     // UPDATED: Handle booking status update with toast
//     const handleUpdateBookingStatus = async (bookingId, newStatus) => {
//         try {
//             const booking = allBookings.find(b => b.id === bookingId);
//             if (!booking) return;
...
//             await fetchAgentBookings(); // Refresh bookings
...
//     // UPDATED: Handle payment status update with toast
//     const handleUpdatePaymentStatus = async (bookingId, newPaymentStatus) => {
//         try {
//             const booking = allBookings.find(b => b.id === bookingId);
//             if (!booking) return;
...
//                     break;
//                 case 'UNPAID':
//                     successMessage = 'Payment status updated to unpaid';
//                     break;
//                 case 'UNPAID_CASH_PICKUP':
//                     successMessage = 'Payment method set to cash on pickup';
//                     break;
//                 case 'CHECKING_BANK_TRANSFER':
//                     successMessage = 'Payment status set to checking bank transfer';
//                     break;
//                 default:
...
const useAgentDashboard = () => {
    ...
    const fetchBankTransferStats = useCallback(async () => {
        try {
            ...
            return {
                pending: agentTransfers.filter(t => t.status === 'Pending').length,
                verified: agentTransfers.filter(t => t.status === 'Verified').length,
                rejected: agentTransfers.filter(t => t.status === 'Rejected').length,
                total: agentTransfers.length
            };
        } catch (err) {
            console.error('Error fetching bank transfer stats:', err);
