...
const BookingCard = ({ booking, onViewDetails, onMakePayment, onDownloadReceipt, onEditBooking }) => {
    ...
    // Helper to get bank transfer status display
    const getBankTransferStatusDisplay = () => {
        if (!booking.bankTransfer) return null;
        
        const status = booking.bankTransfer.status;
        let bgColor = '';
        let textColor = '';
        ...
        return (
            <span className={`px-2 py-1 text-xs rounded-full ${bgColor} ${textColor} ml-2`}>
                BT: {status}
            </span>
        );
    };
            ...
            <div className="p-6">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {getStatusBadge(booking)}
                        {/* Show bank transfer badge if exists */}
                        {getBankTransferStatusDisplay()}
                    </div>
                ...
                {/* Show bank transfer summary if exists */}
                {booking.bankTransfer && (
                    <div className="mt-2 text-xs border-t pt-2">
                        <span className="font-medium text-purple-600">Bank Transfer:</span>
                        <span className="ml-2">
                            {booking.bankTransfer.bankName} - Ref: {booking.bankTransfer.transferReference}
                        </span>
                        <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${
                            booking.bankTransfer.status === 'Verified' ? 'bg-green-100 text-green-700' :
                            booking.bankTransfer.status === 'Rejected' ? 'bg-red-100 text-red-700' :
