...
// src/Pages/Customer/components/BookingUtils.jsx
import axios from 'axios';
...
export const fetchPaymentForBooking = async (bookingId, token) => {
    try {
        ...
        console.debug('Payment not found for booking:', bookingId);
        return null;
    }
};

// NEW: Fetch bank transfer details for a booking
export const fetchBankTransferForBooking = async (bookingId, token) => {
    try {
        const response = await axios.get(`${BASE_URL}/api/v1/banktransfer/getAll`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        
        // Find bank transfer for this booking
        const bankTransfer = response.data.find(bt => bt.bookingId === bookingId);
        return bankTransfer || null;
    } catch {
        console.debug('Bank transfer not found for booking:', bookingId);
        return null;
    }
};
...
export const fetchCustomerBookings = async () => {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            // Enhance bookings with vehicle, agent, payment, and bank transfer details
            const enhancedBookings = await Promise.all(
                customerBookings.map(async (booking) => {
                    // Fetch vehicle details if not present
                    let vehicleData = booking.vehicle;
                    ...
                    // NEW: Fetch bank transfer details
                    const bankTransferData = await fetchBankTransferForBooking(booking.id, token);
                    ...
                    // Return enhanced booking object with bank transfer info
                    return {
                        id: booking.id,
                        bookingStatus: booking.bookingStatus || 'PENDING',
                        paymentStatus: booking.paymentStatus || 'PENDING',
                        ...
                        payment: paymentData,
                        // NEW: Add bank transfer information
                        bankTransfer: bankTransferData ? {
                            id: bankTransferData.id,
                            status: bankTransferData.status, // PENDING, VERIFIED, REJECTED
                            bankName: bankTransferData.bankName,
                            accountNumber: bankTransferData.accountNumber,
                            accountHolder: bankTransferData.accountHolder,
                            transferReference: bankTransferData.transferReference,
                            paymentSlip: bankTransferData.paymentSlip,
                            createdAt: bankTransferData.createdAt
                        } : null
                    };
                })
            );
...
// UPDATED: Include bank transfer status in the badge
export const getStatusBadge = (booking) => {
    // Booking status takes precedence for cancelled and completed
    if (booking.bookingStatus === 'CANCELLED') {
        return <span className="px-3 py-1 bg-red-100 text-red-800 rounded-full text-xs font-medium">Cancelled</span>;
    ...
    // Check if there's a bank transfer with specific status
    if (booking.bankTransfer) {
        switch(booking.bankTransfer.status) {
            case 'Pending':
                return <span className="px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-xs font-medium animate-pulse">Bank Transfer Pending</span>;
            case 'Verified':
                return <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">Bank Transfer Verified</span>;
            case 'Rejected':
                return <span className="px-3 py-1 bg-red-100 text-red-800 rounded-full text-xs font-medium">Bank Transfer Rejected</span>;
            default:
                break;
        }
    }
