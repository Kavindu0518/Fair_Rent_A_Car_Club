...
// // src/Pages/Customer/components/DetailsModal.jsx
// import React, { useState } from 'react';

// const DetailsModal = ({ booking, onClose, onMakePayment, onCancelClick, onDownloadReceipt, onEditClick }) => {
//     const [imageError, setImageError] = useState(false);
//     const BASE_URL = 'http://localhost:8080';
...
//                                             onEditClick(booking);
//                                         }}
//                                         className="flex-1 px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition duration-200 text-center"
//                                     >
//                                         Edit Booking
//                                     </button>
//                                 )}
...
const DetailsModal = ({ booking, onClose, onMakePayment, onCancelClick, onDownloadReceipt, onEditClick }) => {
    ...
    const getStatusBadge = () => {
        ...
        // Check for bank transfer status
            ...
            <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        ...
                        {/* Show bank transfer info badge */}
                        {booking.bankTransfer && booking.bankTransfer.status === 'Verified' && (
                    ...
                    <div className="bg-teal-50 p-4 rounded-lg mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {/* NEW: Bank Transfer Details */}
                            {booking.bankTransfer && (
                                <>
                                    <div className="col-span-2 mt-2 pt-2 border-t border-teal-200">
                                        <h5 className="font-medium text-teal-800 mb-2">Bank Transfer Details</h5>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <p className="text-xs text-gray-600">Transfer Status</p>
                                                <p className={`font-medium ${
                                                    booking.bankTransfer.status === 'Verified' ? 'text-green-600' :
                                                    booking.bankTransfer.status === 'Rejected' ? 'text-red-600' :
                                                    'text-yellow-600'
                                                }`}>
                                                    {booking.bankTransfer.status}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600">Bank Name</p>
                                                <p className="font-medium">{booking.bankTransfer.bankName}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600">Account Holder</p>
                                                <p className="font-medium">{booking.bankTransfer.accountHolder}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-gray-600">Reference</p>
                                                <p className="font-mono text-sm">{booking.bankTransfer.transferReference}</p>
                                            </div>
                                        </div>
                                    </div>
                                </>
